...
function renderClientKPIs(clients) {
    ...
    const kpi = (href, label, value, footer, bg, tMain, tSub, pBg, pCol, pill) =>
        `<a href="${href}" style="background:${bg};border-radius:12px;padding:14px 18px;display:flex;align-items:center;gap:14px;text-decoration:none;transition:filter .15s"
    ...
    document.getElementById('clientKpis').innerHTML =
        kpi('facturacion.php', 'Ingresos Cobrados', fmtNum(cobrado),   `${fmtNum(porCobrar)} por cobrar · COP`,
            '#0f172a', '#ffffff', 'rgba(255,255,255,0.5)', 'rgba(255,255,255,0.15)', '#ffffff', 'Total')
      + kpi('finanzas.php',    'MRR Servicios',     fmtNum(mrr),       `${activos} clientes activos · COP`,
            '#c9f31d', '#0f172a', 'rgba(0,0,0,0.45)',      'rgba(0,0,0,0.15)',       '#0f172a', 'Mensual')
      + kpi('clientes.php',    'En Mora',           mora,              'Requieren atención',
            '#b91c1c', '#ffffff', 'rgba(255,255,255,0.6)', 'rgba(255,255,255,0.2)',  '#ffffff', 'Mora')
      + kpi('clientes.php',    'Total Cartera',     total,             `${pct}% activos`,
            '#334155', '#ffffff', 'rgba(255,255,255,0.5)', 'rgba(255,255,255,0.15)', '#ffffff', 'Clientes');
}
...
function renderClients(clients) {
    ...
    tbody.innerHTML = filtered.map(c => {
        ...
        const freqColors = { mes:'#dbeafe:#2563eb', trimestre:'#ede9fe:#7c3aed', semestre:'#fef3c7:#d97706', año:'#dcfce7:#16a34a', unico:'#f1f5f9:#64748b', ninguna:'#f1f5f9:#94a3b8' };
        const freqList   = [...new Set((c.frecuencias_activas || '').split(', ').map(f => f.trim()).filter(Boolean))];
        const freqBadges = freqList.length
            ? freqList.map(f => {
                const [bg, color] = (freqColors[f] || '#f1f5f9:#64748b').split(':');
                return `<span style="background:${bg};color:${color};padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700;white-space:nowrap">${freqMap[f] || f}</span>`;
              }).join(' ')
        ...
        const negChips   = negNombres.map((n, i) =>
            `<a href="cliente_detalle.php?id=${c.id}"
                style="display:inline-flex;align-items:center;gap:4px;background:#e0f2fe;color:#0369a1;padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700;text-decoration:none;white-space:nowrap;transition:background .12s"
        ...
        <tr class="animate-fade-in" data-id="${c.id}">
            ...
            <td>
                <div style="display:flex;align-items:center;gap:6px">
                    ${c.persona_contacto && (marcasPorContacto[c.persona_contacto] || 0) > 1
                        ? `<svg width="12" height="12" fill="none" stroke="#a5b4fc" viewBox="0 0 24 24" stroke-width="2" title="${marcasPorContacto[c.persona_contacto]} marcas" style="flex-shrink:0"><rect x="2" y="3" width="7" height="7" rx="1"/><rect x="15" y="3" width="7" height="7" rx="1"/><rect x="2" y="14" width="7" height="7" rx="1"/><rect x="15" y="14" width="7" height="7" rx="1"/></svg>`
                        : ''}
                    <span style="font-weight:600;font-size:13px;color:#0f172a">${contacto ? escapeHtml(contacto) : '<span style="color:#94a3b8">—</span>'}</span>
                </div>
                ${c.responsable && c.persona_contacto && c.responsable !== c.persona_contacto
                    ? `<div style="font-size:10px;color:#94a3b8;margin-top:2px">${escapeHtml(c.responsable)}</div>` : ''}
            </td>
            ...
            <td style="text-align:right">
                ...
                ${cobrado > 0 ? `<div style="font-size:10px;color:#64748b;font-weight:600">${fmtNum(cobrado)} cobrado</div>` : ''}
                ${porCobrar > 0 ? `<div style="font-size:10px;color:#f59e0b;font-weight:700">${fmtNum(porCobrar)} p/cobrar</div>` : ''}
            </td>
            <td>
                ${telClean ? `<a href="https://wa.me/${telClean}" target="_blank" style="color:var(--color-success);font-weight:600;font-size:13px;display:flex;align-items:center;gap:4px">
                    ...
                    ${escapeHtml(c.telefono)}</a>`
                : '<span style="color:var(--color-text-muted);font-size:12px">—</span>'}
            </td>
            <td><div style="display:flex;flex-wrap:wrap;gap:4px">${freqBadges}</div></td>
            <td style="text-align:right;white-space:nowrap">
                <button class="btn btn-ghost btn-icon sm" onclick="abrirModalMsgCliente(${c.id})" title="Enviar mensaje"
                    style="margin-right:4px;color:#4f46e5">
...
function buscarContactoExistente(q) {
    ...
    if (!matches.length) {
        wrap.innerHTML = `<div style="padding:11px 14px;font-size:12px;color:#94a3b8;text-align:center">Sin contactos con ese nombre — se creará nuevo</div>`;
    ...
    wrap.innerHTML = matches.map(c => {
        ...
        <div onclick="seleccionarContactoExistente(${c.id})"
          ...
          <div style="width:34px;height:34px;border-radius:9px;background:#0f172a;color:#c9f31d;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:15px;flex-shrink:0">${letra}</div>
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:800;color:#0f172a">${escapeHtml(c.persona_contacto || c.nombre_comercial)}</div>
            <div style="font-size:11px;color:#64748b">${escapeHtml(c.nombre_comercial)}${c.ubicacion ? ' · ' + escapeHtml(c.ubicacion) : ''}</div>
            ${negs.length ? `<div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:3px">${negs.map(n=>`<span style="background:#e0f2fe;color:#0369a1;padding:1px 7px;border-radius:20px;font-size:10px;font-weight:700">${escapeHtml(n)}</span>`).join('')}</div>` : ''}
          </div>
          <span style="flex-shrink:0;background:#c9f31d;color:#0f172a;border-radius:7px;padding:4px 10px;font-size:11px;font-weight:800">Usar</span>
        </div>`;
    }).join('');
    wrap.style.display = 'block';
}
...
async function loadUnicosTransacciones() {
    ...
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:40px;color:#94a3b8;font-size:13px">Cargando pagos únicos…</td></tr>';
    try {
...
function renderUnicoClients() {
    ...
    tbody.innerHTML = data.map(t => {
        ...
        const ec      = ecMap[t.estado] || { bg:'#f1f5f9', color:'#64748b', label: t.estado };
        ...
        return `<tr style="border-bottom:1px solid #f1f5f9;transition:background .12s"
            ...
            <td style="padding:12px 14px">
                <div style="font-weight:700;color:#0f172a;font-size:13px">${escapeHtml(nombre)}</div>
            </td>
            <td style="padding:12px 14px">
                <div style="font-size:13px;color:#475569;font-weight:600">${escapeHtml(t.concepto || '—')}</div>
                ${t.titulo && t.titulo !== t.concepto ? `<div style="font-size:11px;color:#94a3b8;margin-top:1px">${escapeHtml(t.titulo)}</div>` : ''}
            </td>
            <td style="padding:12px 14px;text-align:right;white-space:nowrap">
                <div style="font-weight:900;font-size:14px;color:#16a34a">${monto}</div>
                <div style="font-size:10px;color:#94a3b8">COP</div>
            </td>
            <td style="padding:12px 14px">
                <span style="font-size:11px;font-weight:700;background:${ec.bg};color:${ec.color};padding:3px 10px;border-radius:20px;white-space:nowrap">${ec.label}</span>
            </td>
            <td style="padding:12px 14px;font-size:12px;color:#64748b;white-space:nowrap">${fecha}</td>
            <td style="padding:12px 14px">${waBtn}</td>
            <td style="padding:12px 8px;text-align:right">
                ${cid ? `<button onclick="window.location.href='cliente_detalle.php?id=${cid}'"
                    title="Ver cliente"
                    style="display:inline-flex;align-items:center;padding:5px 10px;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:7px;font-size:11px;font-weight:700;color:#475569;cursor:pointer;transition:all .12s"
...
function renderMarcasGrid(marcas) {
    ...
    grid.innerHTML = marcas.map(c => {
        ...
        const dotColor = c.estado === 'activo' ? '#22c55e' : c.estado === 'en_mora' ? '#ef4444' : '#94a3b8';
        ...
        <div onclick="window.location.href='cliente_detalle.php?id=${c.id}'"
            style="display:flex;align-items:center;gap:12px;padding:12px 14px;border:1.5px solid #e2e8f0;border-radius:12px;cursor:pointer;transition:all .15s;background:#fff"
            onmouseenter="this.style.borderColor='#c9f31d';this.style.background='#fafff5'"
            onmouseleave="this.style.borderColor='#e2e8f0';this.style.background='#fff'">
            <div style="width:40px;height:40px;border-radius:10px;background:#0f172a;color:#c9f31d;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:17px;flex-shrink:0">
                ${inicial}
            </div>
            <div style="flex:1;min-width:0">
                <div style="display:flex;align-items:center;gap:7px;margin-bottom:3px">
                    <span style="font-size:13px;font-weight:800;color:#0f172a">${escapeHtml(c.nombre_comercial)}</span>
            ...
            <svg width="15" height="15" fill="none" stroke="#94a3b8" viewBox="0 0 24 24" stroke-width="2.5" style="flex-shrink:0"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
        </div>`;
    }).join('');
}
